...
import { BarChart, Bar, LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, ScatterChart, Scatter, CartesianGrid, Legend } from "recharts";
import Skeleton from "react-loading-skeleton";
import 'react-loading-skeleton/dist/skeleton.css';

const COLORS = ["#f87171", "#fb923c", "#facc15", "#4ade80", "#60a5fa", "#a78bfa", "#f472b6", "#34d399", "#fbbf24"];
...
const WildfireDashboard = () => {
  ...
  const getAvg = (key) => {
    ...
    const sum = predictions.reduce((acc, p) => acc + (key === "prediction" ? p.prediction : p.input[key] || 0), 0);
  ...
  const prepareMainChartData = () => {
    return predictions.map((p) => ({
      ...
      value: selectedMainMetric === "prediction" ? p.prediction : p.input[selectedMainMetric] || 0,
    }));
  };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold text-center font-serif mb-10">Wildfire Analytics Dashboard</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          ...
          <div className="bg-yellow-100 p-6 rounded shadow text-center">
            <p className="text-lg font-semibold text-yellow-700">Avg. Fire Size</p>
            <p className="text-3xl font-bold">{getAvg("prediction")}</p>
          </div>
          <div className="bg-blue-100 p-6 rounded shadow text-center">
            <p className="text-lg font-semibold text-blue-700">Avg. Temperature</p>
            <p className="text-3xl font-bold">{getAvg("temperature")}
</p>
          </div>
          <div className="bg-green-100 p-6 rounded shadow text-center">
            <p className="text-lg font-semibold text-green-700">Avg. Wind Speed</p>
      ...
      <div className="bg-white p-6 rounded shadow mb-12">
        <div className="flex justify-between mb-4">
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={() => setMainChartType(mainChartType === "line" ? "bar" : "line")}
      ...
      {/* Categorical & Numeric Charts */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        {/* Left Categorical Chart */}
        <div className="bg-white p-6 rounded shadow">
          ...
          <ResponsiveContainer width="100%" height={250}>
            {categoricalChartType === "bar" ? (
              ...
              <PieChart>
                <Pie data={prepareCategoricalData()} dataKey="value" nameKey="name" outerRadius={80}>
